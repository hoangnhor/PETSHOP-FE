...
const OderPage = () => {
    ...
    const cartColumns = [
        {
            ...
            render: (name, record) => (
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    {record.image ? (
                        <img src={record.image} alt={name} style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 6 }} />
                    ) : (
                        <div style={{ width: 64, height: 64, borderRadius: 6, display: "grid", placeItems: "center", background: "linear-gradient(145deg, #f8f5f0, #ecdfca)", color: "#A67C52", fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>Khung ảnh</div>
                    )}
                    <span>{name}</span>
                </div>
            ),
        ...
        { title: "Ngày tạo", dataIndex: "createdAt", render: (value) => (value ? new Date(value).toLocaleString("vi-VN") : "") },
        {
            ...
            render: (_, record) =>
                ["pending", "confirmed"].includes(record.orderStatus) ? (
                    <Popconfirm title="Hủy đơn hàng này?" okText="Hủy đơn" cancelText="Không" onConfirm={() => cancelOrderMutation.mutate(record._id)}>
    ...
    return (
        <>
            <PageContainer title="Thanh toán nhanh" subtitle="Hoàn tất đơn hàng với trải nghiệm premium từ petshop">
                <Row gutter={[20, 20]}>
                    <Col xs={24} lg={15}>
                        <Card title="Giỏ hàng" style={{ borderRadius: 18 }}>
                            {cartItems.length ? (
                                <>
                                    <Table rowKey="idsp" columns={cartColumns} dataSource={cartItems} pagination={false} scroll={{ x: 760 }} />
                    ...
                    <Col xs={24} lg={9}>
                        <Card title="Thông tin nhận hàng" style={{ borderRadius: 18 }}>
                            <Loading isPending={createOrderMutation.isPending}>
                                ...
                                >
                                    ...
                                    <Button type="primary" htmlType="submit" block disabled={!cartItems.length} style={{ height: 44, fontWeight: 600, background: "#1A1A1A", borderColor: "#1A1A1A", borderRadius: 12 }}>
                                        Đặt hàng
                                    </Button>
                                </Form>
                            </Loading>
                ...
                <Card title="Đơn hàng của tôi" style={{ marginTop: 20, borderRadius: 18 }}>
                    <Table rowKey="_id" loading={ordersQuery.isLoading} columns={orderColumns} dataSource={ordersQuery.data?.data || []} scroll={{ x: 840 }} />
                </Card>
            </PageContainer>
</>
    );
